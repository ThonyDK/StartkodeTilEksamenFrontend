...
/*
export function makeRows(dataToMakeRowsWith) {

    const rowsCreated = dataToMakeRowsWith.forEach(data => {`
        <tr>
            <td>${data.navn}</td>
            <td>${data.partimedlemskab.partiNavn}</td>
        </tr>`
    }).join()
    document.getElementById("rows").innerHTML = rowsCreated
}*/
